...
import type { AdminHospitalPharmacistListItem } from "@/libs/type";
import { FiUserPlus } from "react-icons/fi";

function getStatusLabel(status: AdminHospitalPharmacistListItem["status"]) {
  return status === "suspended" ? "Suspended" : "Active";
}

type Props = {
  rows: AdminHospitalPharmacistListItem[];
  search: string;
  isLoading?: boolean;
  onSearchChange: (value: string) => void;
  onOpenCreateModal: () => void;
  onEdit: (pharmacist: AdminHospitalPharmacistListItem) => void;
  onToggleStatus: (pharmacist: AdminHospitalPharmacistListItem) => void;
};

function AdminHospitalPharmacistsListSection({
  rows,
  search,
  isLoading = false,
  onSearchChange,
      ...
      <div className="flex flex-col gap-4 border-b border-line-subtle p-5 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-slate-400">
            Create and manage pharmacist users for this hospital
          </p>
        </div>
              ...
              : rows.length === 0
                ...
                : rows.map((pharmacist) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        <StatusPill status={getStatusLabel(pharmacist.status)} />
                      </td>
                      <td className="p-3">
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => onToggleStatus(pharmacist)}
...
export default AdminHospitalPharmacistsListSection;
