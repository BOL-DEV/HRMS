...
export default function HospitalPharmacyStorePage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (response) => {
      toast.success(response.message || "Pharmacy store created successfully.");
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : "Unable to create pharmacy store.");
    },
  });

  const units = (unitsQuery.data?.data ?? []).filter((unit) => unit.type === "store");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100">Pharmacy Store</h2>
          <p className="text-sm text-gray-600 dark:text-slate-400">
            Configure the hospital&apos;s central warehouse for stock management and dispatches
          </p>
        </div>
              ...
              {unitsQuery.isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="p-8 text-center text-sm text-gray-500 dark:text-slate-400">
                    No central pharmacy store configured for this hospital.
                  </td>
                </tr>
              ) : (
                units.map((unit) => (
                  <tr key={unit.id} className="border-b border-line-subtle text-slate-800 dark:text-slate-200">
                    ...
                    <td className="p-3 font-medium text-amber-600 dark:text-amber-400">Central Store</td>
                    <td className="p-3">
          ...
          <div className="w-full max-w-md rounded-2xl border border-line-subtle bg-panel shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-line-subtle p-5">
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-slate-400">
                  Add the main central pharmacy store used for stock and dispatches.
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-5">
              <label className="block space-y-2">
                <span className="text-sm font-medium text-gray-700 dark:text-slate-300">Store Name</span>
                <input
                  type="text"
