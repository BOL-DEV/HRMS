...
import { getPharmacyUnits, createPharmacyUnit } from "@/libs/pharmacy-api";
...
export default function HospitalPharmacyUnitsPage() {
  ...
  }, [unitsQuery.error, router]);

  const createMutation = useMutation({
    mutationFn: createPharmacyUnit,
    onSuccess: (response) => {
      toast.success(response.message || "Pharmacy unit created successfully.");
      queryClient.invalidateQueries({
        queryKey: ["admin-hospital-pharmacy-units", hospitalId],
      });
      setIsModalOpen(false);
      setNewUnitName("");
      setNewUnitType("point");
    },
    onError: (error) => {
      toast.error(
        error instanceof Error ? error.message : "Unable to create pharmacy unit."
      );
    },
  });
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100">
            Pharmacy Units (Stores & Points)
          </h2>
          <p className="text-sm text-gray-600 dark:text-slate-400">
            Configure central stores and point pharmacy outlets for patient billing
          </p>
        </div>
              ...
              {unitsQuery.isLoading
                ...
                ? (
                    <tr>
                      ...
                      >
                        No pharmacy units configured for this hospital.
                      </td>
                    </tr>
                  )
                : units.map((unit) => (
                    <tr
                      key={unit.id}
                      className="border-b border-line-subtle text-slate-800 dark:text-slate-200"
                    >
                      ...
                      <td className="p-3 capitalize font-medium">
                        {unit.type === "store" ? (
                          <span className="text-amber-600 dark:text-amber-400">Central Store</span>
                        ) : (
                          <span className="text-sky-600 dark:text-sky-400">Pharmacy Point</span>
                        )}
                      </td>
                      <td className="p-3">
          ...
          <div className="w-full max-w-md rounded-2xl border border-line-subtle bg-panel shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-line-subtle p-5">
              <div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100">
                  Create Pharmacy Unit
                </h3>
                <p className="text-sm text-gray-600 dark:text-slate-400">
                  Add a new warehouse central store or point clinic pharmacy.
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-5">
              <label className="block space-y-2">
                ...
                <input
                  type="text"
                  placeholder="e.g. Pharmacy A, Main Store"
                  value={newUnitName}
              ...
              <label className="block space-y-2">
                ...
                <select
                  value={newUnitType}
                  onChange={(e) => setNewUnitType(e.target.value as "store" | "point")}
                  className="w-full rounded-lg border border-line-subtle bg-canvas-alt px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500 dark:text-slate-100"
                >
                  <option value="point">Pharmacy Point (Patient Dispensing Outlet)</option>
                  <option value="store">Central Warehouse Store (Suppliers & Dispatches)</option>
                </select>
              </label>
